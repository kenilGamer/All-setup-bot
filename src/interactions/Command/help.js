...
module.exports = {
  ...
  run: async (client, interaction, args) => {
    ...
    let em1 = new EmbedBuilder()
      .setAuthor({ name: `${client.user.username}\'s Help Menu`, iconURL: client.user.displayAvatarURL({ format: "png" }), url: "https://discord.gg/godcraft" })
...
let startButton = new ButtonBuilder().setStyle(2).setEmoji(`1054428533033816204`).setCustomId('start'),
      ...
      link = new ButtonBuilder().setStyle(5).setLabel("desktop").setEmoji(`992411238644584488`).setURL('https://godcrafts.fun')
    ...
    const allButtons = [startButton.setDisabled(true), backButton.setDisabled(true), forwardButton.setDisabled(false), endButton.setDisabled(true), link]
    ...
    require("fs").readdirSync(`${process.cwd()}/src/commands`).forEach(dirs => {embeds.push(new EmbedBuilder().setAuthor({name: toUpperCase(dirs),iconURL: client.user.displayAvatarURL({format:"png"}),url:"https://godcrafs.fun"}).setDescription(`${commad(dirs)}`))})
